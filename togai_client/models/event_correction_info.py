...
class EventCorrectionInfo(BaseModel):
    """
    EventCorrectionInfo
    """ # noqa: E501
    ...
    status: StrictStr = Field(description="Status of the event requested for correction")
    reason: StrictStr = Field(description="Status description of the event requested for correction")
    __properties: ClassVar[List[str]] = ["referenceId", "eventPayload", "ingestionStatus", "customerId", "source", "createdAt", "status", "reason"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['REVERTED', 'REVERTED_AND_REINGESTED', 'FAILED']):
            raise ValueError("must be one of enum values ('REVERTED', 'REVERTED_AND_REINGESTED', 'FAILED')")
        return value
